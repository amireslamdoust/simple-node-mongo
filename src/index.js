...
const resolvers = {
  Query: {
    ...
    getToDo: async (_, {id}, {db, user}) => {
      ...
      // Fetch the task list with the provided ID
      const todo = await db.collection('ToDo').findOne({_id: new ObjectId(String(id))});
      ...
      const taskList = await db.collection('TaskLists').findOne({_id: todo.taskListId});
  ...
  Mutation: {
    ...
    updateToDo: async (_, {input}, {db, user}) => {
      ...
      // Fetch the updated document
      return await db.collection('ToDo').findOne({_id: new ObjectId(String(id))});
    ...
    deleteToDo: async (_, {id}, {db, user}) => {
      ...
      const result = await db.collection('ToDo').deleteOne({_id: new ObjectId(String(id))})
  ...
  ToDo: {
    id: ({_id, id}) => _id || id,
    taskList: async ({taskListId}, _, {db}) => await db.collection('TaskLists').findOne({_id: new ObjectId(String(taskListId))}),

  },
  TaskList: {
    id: ({_id, id}) => _id || id,
    progress: async ({_id}, _, {db}) => {
      const todos = await db.collection('ToDo').find({taskListId: new ObjectId(String(_id))}).toArray()
    ...
    todos: async ({_id}, _, {db}) => (
      await db.collection('ToDo').find({taskListId: new ObjectId(String(_id))}).toArray()
    )
  }
